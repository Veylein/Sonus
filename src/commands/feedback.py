...
def register(bot):
    ...
    async def feedback_prefix(ctx: commands.Context, *, text: str):
    ...
    # Slash command
    @bot.tree.command(name="feedback", description="Send feedback to the devs")
    async def feedback_slash(interaction: discord.Interaction, text: str):
        channel = bot.get_channel(FEEDBACK_CHANNEL_ID)
        if not channel or not isinstance(channel, discord.TextChannel):
